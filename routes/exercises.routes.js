...
router.post('/by-muscle-groups', isAuthenticated,  async (req, res) => {
    ...
    try {
        ...
        const exercises2 = await Exercise.aggregate([{ $match: { 'belongsTo': { _id: { $in: muscleGroupIds } } } }]).limit(5)
        console.log('is this final ?', exercises2)

        const exercises = await Exercise.find({ 'belongsTo._id': { $in: muscleGroupIds } }).limit(5)
